...
def perform_analysis(compare_to, request, user):
    ...
    try:
        ...
        most_recent_entry = TextEntry.objects.none() # ADD A CHECK/CATCH FOR USERS WHO DON'T HAVE AN ENTRY
        target_frequencies = {}
        target_size = 0
    ...
    if compare_to == 'class':
        ...
        # ADD A CHECK/CATCH FOR USERS WHO DON'T HAVE A CLASS ID
        if hasattr(user.userprofile, 'class_id') and user.userprofile.class_id:
